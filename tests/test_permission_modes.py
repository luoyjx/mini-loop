...
the same three-mode reduction fits in one hook:

    readonly     write/exec/external and unclassified deny outright
    interactive  the default: external and unclassified ask (round 96 broker)
    auto         ask-rules auto-allow, audited
...
(workspace boundary) hold in all three modes. And `readonly` denies rather
...
def test_an_unknown_mode_is_rejected_at_creation(tmp_path):
    ...
    assert PERMISSION_MODES == ("readonly", "interactive", "auto")
